...
import { useGetAdminPlaceQuery, useEditAdminPlaceMutation } from '../../features/placesApi';
...
const EditPage = () => {
  const { id } = useParams();
  const { data, isLoading, isError, isSuccess: dataSuccess } = useGetAdminPlaceQuery(id);
  const [editAdminPlace, { isSuccess }] = useEditAdminPlaceMutation();
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    console.log(edit);
    await editAdminPlace({ id, edit });
    setTimeout(() => navigate('/secretadminpanel/vietos'), 1500);
  }
  ...
  <div className="container-fluid" style={{ marginTop: '64px' }}>
    ...
    <Snackbar onClose={handleSnack} open={openSnack} autoHideDuration={1500} anchorOrigin={{ vertical: 'top', horizontal: 'center' }}>
